...
/**
 * 将 daemon 进程的全部 stdout/stderr 输出同步落盘到 ~/.agentdog/daemon-YYYYMMDD.log。
 *
 ...
 */
export function startDaemonFileLogging(agentdogDir?: string): string {
  if (installed) {
    return getDaemonLogFile(agentdogDir);
  }
  installed = true;

  const dir = agentdogDir || path.join(os.homedir(), '.agentdog');
  try {
 ...
 */
export function getDaemonLogFile(agentdogDir?: string): string {
  const dir = agentdogDir || path.join(os.homedir(), '.agentdog');
